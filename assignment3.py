...
class Assignment3:
    ...
    def integrate(self, f: callable, a: float, b: float, n: int) -> np.float32:
        ...
        result = h / 3 * np.sum(y[0:-1:2] + 4 * y[1::2] + y[2::2], dtype=np.float32)
    ...
    def areabetween(self, f1: callable, f2: callable) -> np.float32:
        intersections = np.sort(Assignment2().intersections(f1, f2, 1, 100))
        if len(intersections) < 2:
            return np.nan
        positive_func = lambda x: abs(f1(x)-f2(x))
        total_area = self.integrate(positive_func, intersections[0], intersections[-1], 1000)
        """
...
#         ass3 = Assignment3()
#         f1 = np.poly1d([-1, 0, 1])
#         f1 = lambda x: 5*math.e**(-x**2)
#         #f1 = lambda x: np.log(6*np.log(x))
#         #f1 = lambda x: (math.e**(-x))/x
#         #f1 = lambda x: x**(math.e - 1) * math.e**(-x)
#         r = ass3.integrate(f1, 0, 4, 4)
#         print("my_answer = ", r)
